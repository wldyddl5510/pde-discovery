...
Definitions and usage for the estimators in `methods.py`.
See [README.md](README.md) for setup and [results.md](results.md) for numerical results.
...
beta_wendy_ols = wendy(*inputs, rho_1=0)
...
beta_mle = wendy_mle(
    *inputs, noise_std=data.noise_std, rho_1=0,
)
```
...
`experiments.py --methods` also accepts `wendy-lasso`, `wendy-ols`,
`wendy-mle-lasso`, and `wendy-mle`. The nonsparse choices are
`wendy-ols` (unpenalized GLS/IRLS) and `wendy-mle` (unpenalized Gaussian MLE).
Both set `rho_1=0` and use the full library without thresholding; MLE retains
its covariance-dependent likelihood and log determinant. WENDy/MLE MSTLS
extensions described above remain available through the Python functions but
are excluded from this comparison. Use `--noise-ratios 1` when comparing
...
Without a working variance, noiseless MLE cells are reported as `N/A`.
...
`--timeout-seconds` limits each complete estimator call, including warm-up
(Unix only). Failed fits and timeouts are recorded with their reason and
elapsed time, and the remaining experiments continue. Returned estimates with
warnings are marked in the tables; a normality stop does not imply fixed-point
convergence. Successful fits use the median of the requested timed repetitions.
